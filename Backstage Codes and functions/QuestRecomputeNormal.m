function q=QuestRecomputeNormal(q, plotIt)
...
if nargin < 1
	error('Usage: q=QuestRecomputeNormal(q [,plotIt=0])')
end
...
q.p2=q.gamma+(1 - q.gamma - q.delta).*normcdf(q.x2,q.tGuess,q.tGuessSd);
...
q.p2=q.gamma+(1 - q.gamma - q.delta).*normcdf(q.x2+q.xThreshold,q.tGuess,q.tGuessSd);
if any(~isfinite(q.p2))
	q %#ok<NOPRT>
	error('psychometric function p2 is not finite')
end
